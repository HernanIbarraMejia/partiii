...
\begin{document}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\end{enumerate}
		\item Let $M = F(\eta)$. It suffices to show that there are at most $|\mathcal{L}|$ many sets $\tp^M(\bar{a}/M)$ as $\bar{a}\in [\eta]^n$ ranges. Since $\eta$ generates $M$ we have that any $\bar{a}$ can be written as an $n$-tuple of $\mathcal{L}$-terms closed by some choice of elements of $\eta$. By indiscernibility, the complete type of $\bar{a}$ does not depend on the choice of elements of $\eta$, only on the terms. And there are at most $n\times|\mathcal{L}| = |\mathcal{L}|$ of choosing these. 
		\end{enumerate}
\end{document}
