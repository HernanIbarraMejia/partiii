	...
	\begin{proof}
		...
		\begin{proof}
 		...
 		\begin{thm}\label{thm:ax_ex_cl_imp_qe}
 			Let $\mathbb{K}$ be a class of $\mathcal{L}$-structures closed under isomorphisms and such that the class of all the substructures of $\mathbb{K}$-structures has the amalgamation property. 
 			
 			Then for every existential $\mathcal{L}$-formula $\varphi(x_1,\ldots,x_n)$ there is a family of sets $\{S_{\alpha}\}_{\alpha\in I}$ with the following properties.
 			\begin{enumerate}
 				\item For all $\alpha\in I$ we have that $S_{\alpha}$ is a set of $\mathcal{L}$-literals with free variables in $\{x_1,\ldots, x_n\}$.
 				\item For all existentially closed $M\in \mathbb{K}$ and $\bar{m}\in M^n$ we have
 				\[
 					M\models \varphi(\bar{m}) \iff \text{There is some $\alpha\in I$ such that $M\models \psi(\bar{m})$ for all $\psi\in S_\alpha$}.
 				\]
 			\end{enumerate}
 		\end{thm}
 		\begin{proof}
 			Let $\varphi(\bar{x})$ be an existential formula with $\bar{x}= (x_1,\ldots,x_n)$. Define 
 			\[
 				I \coloneqq \{(M,\bar{m}) \mid \text{$M$ is e.c. in $\mathbb{K}$, $\bar{m}\in M^n$, and $M\models \varphi(\bar{m})$}\}.
 			\]
 			For $(M,\bar{m})\in I$ define
 			\[
 				S_{(M,\bar{m})} \coloneqq \{\psi(\bar{x}) \mid \text{$\psi$ is an $\mathcal{L}$-literal and $M\models \psi(\bar{m})$}\}.
 			\]
 			These satisfy condition $(a)$; now we check $(b)$. Let $M$ be e.c. in $\mathbb{K}$ and $\bar{m}\in M^n$. If $M \models \varphi(\bar{m})$ then clearly $\bar{m}$ satisfies all literals in $S_{(M,\bar{m})}$ by definition.
 			
 			Conversely, suppose there is $(N,\bar{n})\in I$ such that $M \models \psi(\bar{m})$ for all $\psi(\bar{x})\in S_{(N,\bar{n})}$. We need to show that $M\models \varphi(\bar{m})$ Expanding definitions, we see that
 			\[
 				\text{$\psi(\bar{x})$ is a literal with $N\models \psi(\bar{n})$} \implies M\models \psi(\bar{m}).
 			\]
 			By induction over the structure of formulae we get the stronger statement:
 			\[
 				\text{$\psi(\bar{x})$ is a q.f. formula with $N\models \psi(\bar{n})$} \implies M\models \psi(\bar{m}).
 			\]
 			It readily follows that $M$ is a model of $\Diag(\langle \bar{n}\rangle_N)$ by interpreting $\bar{m}$ as $\bar{n}$. Hence there is an embedding $e\colon \langle \bar{n}\rangle_N \hookrightarrow M$ such that $e(\bar{n}) = \bar{m}$.
 			
 			As $\langle \bar{n}\rangle_N$ is a substructure of a $\mathbb{K}$-structure (recall $(N,\bar{n})\in I$) we get, by the amalgamation property, a substructure $C$ of a $\mathbb{K}$-structure $C'$, together with embeddings $f,g$ making the following diagram commute.
 			% https://q.uiver.app/#q=
 			\[\begin{tikzcd}[cramped]
 				& {C'} \\
 				& C \\
 				M && N \\
 				& {\langle\bar{n}\rangle_{N}}
 			...
 			\end{tikzcd}\]
 		We have that $(N,\bar{n})\in I$, so $N\models \varphi(\bar{n})$. As $\varphi$ is existential and existential formulae are preserved under extensions, we have that $C' \models \varphi(g(\bar{n}))$. By commutativity of the diagram we have that $g(\bar{n}) = f(e(\bar{n})) = f(\bar{m})$. So $C'\models \varphi(f(\bar{m}))$. By replacing $C'$ with an isomorphic copy, which we can do since $\mathbb{K}$ is closed under isomorphisms, we can assume that $f$ is an inclusion. It follows, as $M$ is e.c. in $\mathbb{K}$, that $M\models \varphi(\bar{m})$ as desired. 
 		\end{proof}
 		\begin{coro}
 			Let $\mathbb{K}$ be a class of $\mathcal{L}$-structures closed under isomorphisms and such that the class of all the substructures of $\mathbb{K}$-structures has the amalgamation property. If $\mathcal{T}$ is a theory axiomatizing existentially closed structures of $\mathbb{K}$ then $\mathcal{T}$ has quantifier elimination.
 		\end{coro}
 		\begin{proof}
 			By syntactic quantifier elimination, it suffices to eliminate quantifiers of existential formulae. Let $\varphi(x_1,\ldots,x_n)$ be existential. Use the previous theorem to obtain the sets $\{S_{\alpha}\}_{\alpha\in I}$.
 			
 			Add new constants $\bar{c}=(c_1,\ldots,c_n)$ to the language and consider the $S_\alpha$'s as $\mathcal{L}_{\bar{c}}$-theories by replacing $\bar{x}$ with $\bar{c}$. Let $\alpha\in I$ be arbitrary. Then we have 
 			\[
 				\mathcal{T} \cup S_\alpha \models \varphi(\bar{c})
 			\]
 			by property $(b)$ in Theorem \ref{thm:ax_ex_cl_imp_qe} and the fact that models of $\mathcal{T}$ are precisely e.c. $\mathcal{K}$-structures. By Compactness and the deduction theorem, there is a finite subset $S'_{\alpha}\subseteq S_{\alpha}$ such that 
 			\[
 				\mathcal{T} \models \bigwedge S_{\alpha}' \rightarrow \varphi(\bar{c}),
 			\]
 			and this is true for all $\alpha\in I$. Let $\psi_\alpha \coloneqq \bigwedge S_{\alpha}'$ and note that $\psi_{\alpha}$ is quantifier free. We now note that 
 			\[
 				\mathcal{T} \cup \{\neg\psi_{\alpha}\}_{\alpha\in I} \models \neg\varphi(\bar{c})
 			\]
 			again by property $(b)$. Again by Compactness there is a finite subset $I'\subseteq I$ such that 
 			\[
 				\mathcal{T} \models \bigwedge_{\alpha\in I'}\neg\psi_{\alpha} \rightarrow \neg\varphi(\bar{c}),
 			\]
 			or equivalently,
 			\[
 			\mathcal{T} \models \varphi(\bar{c}) \rightarrow \bigvee_{\alpha\in I'}\psi_{\alpha}.
 			\]
 			Let $\psi \coloneqq \bigvee_{\alpha\in I'}\psi_{\alpha}$ and note that $\psi$ is quantifier free. We have that $\mathcal{T} \models \varphi(\bar{c}) \rightarrow \psi$.  We prove the converse.
 			
 			If $I'= \emptyset$ then $\psi = \bot$ and thus it follows that $\mathcal{T} \models \varphi(\bar{c}) \leftrightarrow \bot$. Assume that $I'$ is nonempty. If $(M,\bar{m})$ is an $\mathcal{L}_{\bar{c}}$-model of $\mathcal{T}$ and $M\models \psi$ then, as $I'$ is nonempty, there is $\alpha$ such that $M\models \psi_{\alpha}$. But we had $\mathcal{T}\models \psi_{\alpha}\rightarrow \varphi(\bar{c})$ so $M\models \varphi(\bar{m})$. It follows that $\mathcal{T}\models \psi \rightarrow \varphi(\bar{c})$.
 			
 			Hence we have $\mathcal{T} \models \varphi(\bar{c}) \leftrightarrow \psi$, where $\psi$ is quantifier free. But $\bar{c}$ does not appear in $\mathcal{T}$ so, by generalization,
 			\[
 				\mathcal{T} \models \forall\bar{x}.(\varphi(\bar{x}) \leftrightarrow \psi).\qedhere
 			\]
 		\end{proof}
 		\begin{eg}
 			\begin{enumerate}
